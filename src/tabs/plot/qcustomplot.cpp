...
void QCPAbstractPlottable::rescaleValueAxis(bool onlyEnlarge) const
{
  ...
  if (foundRange)
  {
    ...
    if ( newRange.upper > 0)
        newRange.upper *= 1.2;
    else if (newRange.upper < 0)
        newRange.upper *= .8;

    if ( newRange.lower > 0)
        newRange.lower *= .8;
    else if (newRange.lower < 0)
        newRange.lower *= 1.2;

    //qDebug() << newRange.upper << newRange.lower;
  ...
  mReplotting(false)
{
  ...
  defaultAxisRect->insetLayout()->setMargins(QMargins(2, 8, 8, 2));
...
/*!
  ...
  penalty on replots. (Except when the viewport dimensions are changed continuoualphasly.)
  ...
  mDragging(false)
{
  ...
  setMinimumMargins(QMargins(1, 1, 1, 1));
...
QCPPlotTitle::QCPPlotTitle(QCustomPlot *parentPlot) :
  QCPLayoutElement(parentPlot),
  mFont(QFont(QLatin1String("sans serif"), 12*1.2, QFont::Bold)),
  mTextColor(Qt::black),
  mSelectedFont(QFont(QLatin1String("sans serif"), 12*1.3, QFont::Bold)),
  ...
  mSelected(false)
{
  if (parentPlot)
  {
    ...
    mFont = QFont(parentPlot->font().family(), parentPlot->font().pointSize()*1.2, QFont::Bold);
    mSelectedFont = QFont(parentPlot->font().family(), parentPlot->font().pointSize()*1.3, QFont::Bold);
  }
...
QCPRange QCPGraph::getKeyRange(bool &foundRange, SignDomain inSignDomain, bool includeErrors) const
{
  ...
  if (inSignDomain == sdBoth) // range may be anywhere
  {
    ...
    while (it != mData->constEnd())
    {
      ...
      currentErrorMinus = 0;//(includeErrors ? it.value().keyErrorMinus : 0);
      currentErrorPlus = 0;//(includeErrors ? it.value().keyErrorPlus : 0);
...
QCPRange QCPGraph::getValueRange(bool &foundRange, SignDomain inSignDomain, bool includeErrors) const
{
  ...
  if (inSignDomain == sdBoth) // range may be anywhere
  {
    ...
    while (it != mData->constEnd())
    {
      ...
      currentErrorMinus = 0;//(includeErrors ? it.value().valueErrorMinus : 0);
      currentErrorPlus = 0;//(includeErrors ? it.value().valueErrorPlus : 0);
      if (current-currentErrorMinus < range.lower || !haveLower)
      {
        range.lower = current-currentErrorMinus;
        haveLower = true;
